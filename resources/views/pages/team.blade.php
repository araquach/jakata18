...
<section id="team">
	<section id="team_overview">
		<h1>The Team</h1>
		<p>The Jakata team comprises of eleven skilled stylists, all from different hairdressing backgrounds, all displaying a diverse range of skills.</p>
		...
		@foreach($team_members as $team_member)
			<a href="{{ URL::to('team/' . $team_member->class) }}" >
				<section class="team_member" id="{{ $team_member->class }}">
					<h2>{{ $team_member->name }} <br> <span class="small"> {{ $team_member->level }}</span></h2>
				</section>
			</a>
